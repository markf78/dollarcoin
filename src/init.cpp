...
void Shutdown()
{
    ...
    RenameThread("dollarcoin-shutoff");
    mempool.AddTransactionsUpdated(1);
    StopRPCThreads();
    ShutdownRPCMining();
    if (pwalletMain)
        bitdb.Flush(false);
    GenerateDollarcoins(false, NULL, 0);
    StopNode();
    {
        LOCK(cs_main);
        if (pwalletMain)
...
{
    RenameThread("dollarcoin-loadblk");
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Dollarcoin version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (pwalletMain)
        GenerateDollarcoins(GetBoolArg("-gen", false), pwalletMain, GetArg("-genproclimit", -1));
